...
macro_rules! spa {
    ...
    // terminal arm — exclude globs become rust-embed #[exclude] attributes.
    ($struct:ident, $path:expr, $base:expr, [$($index:expr),*], { $($config:tt)* }, exclude: [$($ex:expr),*] $(,)?) => {
        ...
        $crate::__macro_dependencies::paste::paste!{
            ...
            pub mod [<mod_ $struct:lower>] {
                ...
                pub static SPA: std::sync::OnceLock<anycms_spa::actix::ActixSpa<super::$struct>> = std::sync::OnceLock::new();
            }
